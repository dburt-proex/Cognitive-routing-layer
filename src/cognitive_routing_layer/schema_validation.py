"""Standard-library validator for the JSON Schema subset this repository uses.

The runtime carries no third-party dependencies, matching the
operator-intelligence/leverage-engine convention. The supported subset is
type, enum, required, properties, additionalProperties, items, minItems,
uniqueItems, minLength, pattern, format=date-time, minimum and maximum.
`tests/test_acceptance.py` cross-checks every schema document against the
`jsonschema` reference implementation when it is installed, so the subset
cannot silently drift from real JSON Schema semantics.
"""
import re
...
def validate(instance: Any, schema: dict[str, Any], path: str = "$") -> None:
    ...
    if "enum" in schema and instance not in schema["enum"]:
        raise SchemaValidationError(f"{path}: {instance!r} is not in enum {schema['enum']}")
    ...
    if isinstance(instance, str):
        ...
        if pattern and not re.fullmatch(pattern, instance):
            raise SchemaValidationError(f"{path}: {instance!r} does not match {pattern}")
